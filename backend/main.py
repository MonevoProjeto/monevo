from fastapi import FastAPI, HTTPException, Depends, UploadFile, File, Request # ADICIONADO Request
...
# Importações ADICIONADAS para Google OAuth
from fastapi.responses import RedirectResponse
from starlette.middleware.sessions import SessionMiddleware # Necessário para o Authlib
from authlib.integrations.starlette_client import OAuth
from jose import jwt, JWTError # ADICIONADO JWTError (boa prática)
...
from database import (
    ...
    SessionLocal # Mantido
)

#autenticação 
#rotas de login/registro 
#usa token JWT 
from auth import pegar_usuario_atual, criar_hash_senha, criar_token, verificar_hash_senha # ADICIONADO verificar_hash_senha, criar_token
...
ALLOWED_ORIGINS = [
        ...
        "http://localhost:8082",
        "http://127.0.0.1:8082",
        # ADICIONAR ORIGEM DO CALLBACK do Google
        "http://localhost:8000" 
]
...
# --- ADICIONAR MIDDLEWARE DE SESSÃO E CONFIGURAR OAUTH ---
# A SessionMiddleware é NECESSÁRIA para o Authlib gerenciar o estado do OAuth.
app.add_middleware(
    SessionMiddleware, 
    secret_key=JWT_SECRET_KEY, # Usa a mesma chave JWT para assinar o cookie de sessão
    session_cookie="google_oauth_session" # Nome do cookie
)
...
async def google_login(request: Request):
    ...
    # O Redirect URI DEVE ser 'http://localhost:8000/auth/google/callback'
...
async def google_auth_callback(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        # 1. Troca o código por tokens e obtém os dados do usuário
        token = await oauth.google.authorize_access_token(request)
        # user_info vem do ID Token, contém 'email', 'name', 'sub' (Google ID)
        user_info = await oauth.google.parse_id_token(request, token) 
        
        email = user_info.get('email')
        name = user_info.get('name', 'Usuário IBMEC')

        if not email:
            # Se o email não for fornecido, redireciona com erro
            return RedirectResponse(f"{FRONTEND_URL}/auth/callback?error=email_nao_encontrado", status_code=302)

        # 2. Lógica de Detecção de Perfil (REPLICANDO O REQUISITO ORIGINAL)
        if "professor" in email.lower():
            perfil = "Professor"
        else:
            perfil = "Aluno"
        
        # 3. Busca/Criação do Usuário no Banco de Dados
        user = db.query(UsuarioTable).filter(UsuarioTable.email == email).first()

        if not user:
            # Novo usuário - Crie um rascunho e envie para o Onboarding
            # O onboarding tradicional usa senha, mas aqui é social. 
            # Damos uma senha aleatória para satisfazer a constraint, 
            # e forçamos o onboarding para coletar os dados adicionais.
            
            # ATENÇÃO: Se a sua função 'criar_hash_senha' espera uma string, 
            # use uma string temporária ou adapte a lógica.
            ...
            novo_usuario = UsuarioTable(
                email=email, 
                nome=name, 
                senha_hash=senha_hash, # Senha hash temporária
            )
            db.add(novo_usuario)
            db.commit()
            db.refresh(novo_usuario)
            
            # Redirecionamento para o Onboarding, passando o ID do usuário
            return RedirectResponse(f"{FRONTEND_URL}/onboarding?user_id={novo_usuario.id}&oauth=google", status_code=302)
        else:
            user_id = user.id
            
        # 4. Geração do JWT
        # Crio um payload com o perfil detectado
        jwt_payload = {"sub": str(user_id), "email": email, "perfil": perfil}
        # Uso a função criar_token existente no auth.py, ou crio uma se não existir.
        # Assumo que 'criar_token' recebe o ID do usuário e retorna o JWT.
        # Se 'criar_token' não aceitar um payload customizado, você deve ajustá-lo 
        # ou usar a função 'criar_jwt_token' que sugeri antes. 
        
        # Tentativa de usar a função do seu auth.py (pode ser necessário ajuste)
        # Se sua função 'criar_token' precisa de mais parâmetros, use a função utilitária abaixo
        
        token_jwt = jwt.encode(jwt_payload, JWT_SECRET_KEY, algorithm="HS256")
        
        # 5. Redirecionamento de SUCESSO para o Frontend
        return RedirectResponse(f"{FRONTEND_URL}/auth/callback?token={token_jwt}&perfil={perfil}", status_code=302)

    except JWTError as e:
        logger.exception(f"Erro ao processar token JWT do Google: {e}")
        return RedirectResponse(f"{FRONTEND_URL}/auth/callback?error=jwt_falhou", status_code=302)
    except Exception as e:
        logger.exception(f"Erro no Google Callback: {e}")
        # Redirecionamento de ERRO para o Frontend
